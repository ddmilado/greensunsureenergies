    ...
    <div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-4">
      {processSteps.map((step, index) => (
        <Reveal key={step.title} delay={index * 0.08} className="lg:sticky lg:top-32" as="article">
          <div
            data-float-card
            className="min-h-[340px] rounded-[2.25rem] bg-[var(--shell)] p-1.5 ring-1 ring-[var(--line)]"
          ...
          >
            <div className="flex h-full flex-col justify-between rounded-[1.85rem] bg-white p-7">
              ...
              <div>
                <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--ink-950)]">{step.title}</h3>
                <p className="mt-4 text-base leading-7 text-[var(--ink-600)]">{step.text}</p>
              </div>
            </div>
          </div>
        </Reveal>
