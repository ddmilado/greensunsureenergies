      ...
      {services.map((service, index) => {
        ...
        return (
          <Reveal key={service.title} delay={index * 0.05} className={`group ${span}`}>
            <article className="h-full rounded-[2.25rem] bg-[var(--shell)] p-1.5 shadow-[0_24px_90px_rgba(0,39,61,0.08)] ring-1 ring-[var(--line)] transition duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-2 hover:shadow-[0_34px_110px_rgba(0,83,122,0.16)]">
              <div className="flex h-full flex-col justify-between rounded-[1.85rem] bg-white p-7 shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]">
                ...
                <div className="mt-10">
                  <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--ink-950)] md:text-3xl">
                    {service.title}
                  </h3>
                  <p className="mt-4 max-w-md text-base leading-7 text-[var(--ink-600)]">{service.text}</p>
                </div>
              </div>
            </article>
          </Reveal>
